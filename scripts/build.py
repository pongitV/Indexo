...
def build():
    ...
    portable_dir = root / "Portable-EXE" / "Indexo-Portable"
    ...
    # Copy Resources, Guide and create folders
    resources_dest = portable_dir / "resources"
    resources_dest.mkdir(parents=True, exist_ok=True)
    shutil.copy2(root / "resources" / "system_rules.json", resources_dest / "system_rules.json")
    shutil.copy2(root / "resources" / "icon.png", resources_dest / "icon.png")
    shutil.copy2(root / "resources" / "icon.ico", resources_dest / "icon.ico")
    if (resources_dest / "i18n").exists():
        shutil.rmtree(resources_dest / "i18n")
    shutil.copytree(root / "resources" / "i18n", resources_dest / "i18n")

    # Guide
    guide_src = root / "Portable-EXE" / "INDEXO_PORTABLE_GUIDE.md"
    if guide_src.exists():
