...
class IndexWorker(QThread):
    ...
    def run(self):
        try:
            ...
            confidence_threshold = float(self.settings_mgr.get("confidence_threshold", 0.85))
            ...
            for new_tag in discovered_tags:
                if float(new_tag.get("confianca_base", 0.0)) >= 0.65:
            ...
            for idx, entry in enumerate(entries):
                ...
                if file_type == "document":
                    ...
                    try:
                        ...
                        # Use EXIF description/keywords as text content
                        extracted_text = " ".join([
                ...
                # Check if this file belongs to a cohesive bundle (e.g. PEAK/PEAK.exe or assets)
                parent_bundle = file_to_bundle.get(rel_path.replace("\\", "/"))

                # Match against active user & discovered tags (Priority 1: Name pattern / Folder / Entity)
                matching_tag = None
                ...
                for t in all_tags:
                    ...
                    # 1. Folder match: check if ANY ancestor folder matches tag name or synonym
                    ...
                    # 3. Keywords match in filename stem or extracted text
                ...
                # Hierarchical classification: 1. Name/Folder -> 2. Discovered Tag -> 3. Content -> 4. Type
                ...
                file_hash = None
                try:
                    if size < 500 * 1024 * 1024:  # Avoid hashing single huge files > 500MB during fast scan
                ...
                if is_in_bundle:
                    # Keep internal bundle files with their original names intact!
            ...
            logger.info("Scan and classification finished in {:.2f}s ({} identified, {} duplicates, {} pending, {} cohesive bundles)", elapsed, identified_count, duplicates_count, pending_count, len(cohesive_bundles))
            ...
            summary = {
                ...
                "duplicates_groups": duplicates_raw,
            ...
            logger.exception("Error during indexing: {}", e)
