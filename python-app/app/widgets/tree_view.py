...
class VirtualTreeView(QWidget):
    ...
    def init_ui(self):
        ...
        header = QHBoxLayout()
        self.lbl_title = QLabel(f"🏷️ {tr('nav.tags')}")
        ...
        self.btn_manage_tags = QPushButton("Gerenciar Tags")
        self.btn_manage_tags.setStyleSheet("padding: 4px 10px; font-size: 13px;")
        self.btn_manage_tags.clicked.connect(self.tag_manager_requested.emit)
        header.addWidget(self.btn_manage_tags)
    ...
    def prompt_create_user_tag(self):
        name, ok = QInputDialog.getText(self, tr("action.create_tag"), "Nome da nova categoria/tag:")
        if ok and name.strip():
            ...
            QMessageBox.information(self, "Indexo", f"Tag '{name.strip()}' criada com sucesso!")
