...
from PySide6.QtWidgets import (
    ...
    QCheckBox, QMenu, QDialog, QInputDialog
)
...
class PendingListView(QWidget):
    ...
    def init_ui(self):
        ...
        top_bar = QHBoxLayout()
        self.lbl_title = QLabel(f"❓ {tr('pending.title', count=0)}")
        self.lbl_title.setFont(QFont("Inter", 11, QFont.Weight.Bold))
        top_bar.addWidget(self.lbl_title)

        top_bar.addStretch()

        self.btn_accept_all_high = QPushButton(f"⚡ {tr('action.accept_high_confidence')}")
        ...
        self.btn_trash_selected = QPushButton("🗑️ Mover Selecionados para Lixeira")
        ...
        self.search_input.setPlaceholderText("Filtrar arquivos pendentes por nome, tag ou extensão...")
        ...
        self.filter_mode_combo.addItems([
            "Todos os Pendentes",
            "Com Sugestão de Tag",
            "Sem Sugestão (Não Identificados)"
        ])
        ...
        self.table.setHorizontalHeaderLabels([
            "Arquivo", "Sugestão da IA / Regras", "Categoria", "Ações Rápidas"
        ])
        ...
        self.lbl_selected_title = QLabel("Selecione um arquivo acima para revisar e definir sua classificação.")
        ...
        # 1. Aceitar Sugestão
        self.btn_accept_single_sug = QPushButton("✅ Aceitar Sugestão")
        ...
        # 2. Selecionar Tag
        self.btn_select_tag_single = QPushButton("🏷️ Selecionar Tag...")
        ...
        # 3. Criar Tag
        self.btn_create_tag_single = QPushButton("➕ Criar Tag...")
        ...
        self.btn_select_cat_single = QPushButton("📁 Selecionar Categoria...")
        ...
        # 5. Criar Categoria
        self.btn_create_cat_single = QPushButton("📂 Criar Categoria...")
        ...
        self.btn_open_dir = QPushButton("📂 Abrir Pasta")
        ...
        self.btn_trash_single = QPushButton("🗑️ Mover para Lixeira")
    ...
    def trash_selected_batch(self):
        selected_rows = self.table.selectionModel().selectedRows()
        if not selected_rows:
            QMessageBox.information(self, "Indexo", "Selecione ao menos um arquivo na tabela.")
            return
        ...
        reply = QMessageBox.question(
            self,
            "Confirmar Envio para Lixeira",
            f"Deseja marcar os {count} arquivos selecionados para a lixeira virtual?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
        if reply == QMessageBox.StandardButton.Yes:
            ...
            self.lbl_title.setText(f"❓ {tr('pending.title', count=len(self.pending_items))}")
            self.apply_filter()
            QMessageBox.information(self, "Indexo", f"{count} arquivo(s) marcados para a lixeira.")
