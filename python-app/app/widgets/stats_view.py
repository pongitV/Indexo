...
class StatsView(QWidget):
    ...
    def init_ui(self):
        ...
        self.lbl_title = QLabel("Estatísticas & Análise da Pasta")
        ...
        self.card_files = self.create_kpi_card("Total de Arquivos", "0", "Aguardando escaneamento", "#205EA6")
        self.card_size = self.create_kpi_card("Espaço Total em Disco", "0 MB", "Tamanho combinado", "#2E7D32")
        self.card_accuracy = self.create_kpi_card("Precisão Semântica", "0%", "Classificados com alta confiança", "#D97706")
        self.card_duplicates = self.create_kpi_card("Duplicatas & Desperdício", "0 (0 MB)", "Economia potencial de espaço", "#9333EA")
        ...
        type_card = QFrame()
        type_card.setObjectName("settings_panel")
        type_card_layout = QVBoxLayout(type_card)
        type_card_layout.setContentsMargins(16, 16, 16, 16)
        type_card_layout.setSpacing(12)

        lbl_chart_title = QLabel("Distribuição de Espaço por Tipo de Arquivo")
        lbl_chart_title.setFont(QFont("Inter", 11, QFont.Weight.Bold))
        type_card_layout.addWidget(lbl_chart_title)
        ...
        table_card = QFrame()
        table_card.setObjectName("settings_panel")
        table_card_layout = QVBoxLayout(table_card)
        table_card_layout.setContentsMargins(16, 16, 16, 16)
        table_card_layout.setSpacing(12)

        lbl_table_title = QLabel("Detalhamento por Categoria & Extensões")
        lbl_table_title.setFont(QFont("Inter", 11, QFont.Weight.Bold))
        table_card_layout.addWidget(lbl_table_title)

        self.table_stats = SmoothTableWidget(0, 5)
        self.table_stats.setHorizontalHeaderLabels([
            "Categoria / Tipo", "Quantidade", "Tamanho Total", "% do Disco", "Confiança Média"
        ])
    ...
    def create_kpi_card(self, title: str, val: str, sub: str, border_color: str) -> Dict[str, Any]:
        ...
        return {"frame": frame, "value": lbl_v, "sub": lbl_s}
    ...
    def update_stats(self, items: List[Dict[str, Any]], duplicates_count: int = 0, duplicates_bytes: int = 0):
        ...
        self.card_files["sub"].setText(f"{identified_files} identificados | {total_files - identified_files} pendentes")

        self.card_size["value"].setText(format_size(total_bytes))
        avg_size = total_bytes / total_files if total_files > 0 else 0
        self.card_size["sub"].setText(f"Média de {format_size(int(avg_size))} por arquivo")

        acc_pct = (identified_files / total_files * 100) if total_files > 0 else 0
        self.card_accuracy["value"].setText(f"{acc_pct:.1f}%")
        self.card_accuracy["sub"].setText("Arquivos categorizados com sucesso")

        self.card_duplicates["value"].setText(f"{duplicates_count:,} ({format_size(duplicates_bytes)})")
        self.card_duplicates["sub"].setText("Espaço duplicado detectado")

        # 2. Group by Type for Chart
        type_palette = {
            "document": ("#205EA6", "Documentos (PDF, DOCX)"),
            "image": ("#2E7D32", "Imagens (PNG, JPG)"),
            "audio": ("#D97706", "Áudio"),
            "video": ("#9333EA", "Vídeos"),
            "text": ("#0D9488", "Textos & Código"),
            "other": ("#6B7280", "Outros"),
        }
        ...
        for r_idx, (cat_name, st) in enumerate(sorted_cats):
            ...
            self.table_stats.setItem(r_idx, 1, QTableWidgetItem(f"{st['count']} arquivos"))
