...
class SearchPaletteDialog(QDialog):
    ...
    def init_ui(self):
        ...
        search_card = QFrame()
        search_card.setObjectName("card_search_input")
        search_card.setStyleSheet(
            "QFrame#card_search_input { background: rgba(30, 41, 59, 0.95); border: 1px solid #3b82f6; border-radius: 8px; }"
        )
        ...
        self.search_input.setStyleSheet("border: none; background: transparent; color: #f8fafc;")
        ...
        btn_close.setStyleSheet(
            "QPushButton { background: rgba(71, 85, 105, 0.4); color: #cbd5e1; border: 1px solid #475569; border-radius: 6px; padding: 4px 10px; font-weight: bold; }"
        ...
        # 2. Filter Filter Buttons (Todos | 📁 Pastas | 📄 Arquivos)
        filter_bar = QHBoxLayout()
        filter_bar.setSpacing(6)

        self.btn_all = QPushButton("✨ " + ("Todos" if LanguageManager.get_instance().current_language == "ptBR" else "All"))
        self.btn_folders = QPushButton("📁 " + ("Pastas" if LanguageManager.get_instance().current_language == "ptBR" else "Folders"))
        self.btn_files = QPushButton("📄 " + ("Arquivos" if LanguageManager.get_instance().current_language == "ptBR" else "Files"))
        ...
        for btn, mode in [(self.btn_all, "all"), (self.btn_folders, "folders"), (self.btn_files, "files")]:
            ...
            btn.setStyleSheet(
                "QPushButton { padding: 4px 14px; border-radius: 6px; background: rgba(51, 65, 85, 0.6); color: #cbd5e1; border: 1px solid #475569; }"
        ...
        self.results_list.setStyleSheet(
            "QListWidget { background: rgba(15, 23, 42, 0.95); border: 1px solid #334155; border-radius: 8px; padding: 6px; }"
            "QListWidget::item { padding: 8px 10px; border-radius: 6px; color: #f1f5f9; }"
            "QListWidget::item:hover { background: rgba(51, 65, 85, 0.8); }"
        ...
        footer = QHBoxLayout()
        hint_text = (
            "↑↓ Navegar  ·  Enter Selecionar para Organização  ·  Esc Fechar"
            if LanguageManager.get_instance().current_language == "ptBR"
            else "↑↓ Navigate  ·  Enter Select for Organization  ·  Esc Close"
        )
