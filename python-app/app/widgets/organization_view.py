...
class OrganizationSplitView(QWidget):
    ...
    def init_ui(self):
        ...
        self.lbl_before.setFont(QFont("Inter", 12, QFont.Weight.Bold))
        ...
        self.tree_before.setFont(QFont("Inter", 11))
        ...
        self.lbl_after.setFont(QFont("Inter", 12, QFont.Weight.Bold))
        ...
        self.tree_after.setFont(QFont("Inter", 11))
    ...
    def populate_results(
        ...
        self.lbl_summary.setText(tr("view.processed_summary", total=total, identified=identified, pending=pending))
    ...
    def _populate_cohesive_table(self):
        ...
        for row, bundle in enumerate(self.cohesive_bundles):
            ...
            item_cat = QTableWidgetItem(f"🏷️ {cat}")
            ...
            item_files = QTableWidgetItem(f"{file_count} arqs ({size_str})")
            item_files.setFont(QFont("Inter", 9))
            self.table_cohesive.setItem(row, 3, item_files)

            combo = QComboBox()
            combo.addItem(f"📦 {tr('cohesive.action_move_parent')}", "move_parent")
            combo.addItem(f"⏸️ {tr('cohesive.action_keep')}", "keep")
            combo.addItem(f"✂️ {tr('cohesive.action_disassemble')}", "disassemble")
            ...
            combo.currentIndexChanged.connect(lambda idx, r=row: self.on_bundle_action_combo_changed(r, idx))
            self.table_cohesive.setCellWidget(row, 4, combo)

    def on_bundle_action_combo_changed(self, row: int, index: int):
        if row < len(self.cohesive_bundles):
            combo = self.table_cohesive.cellWidget(row, 4)
            if isinstance(combo, QComboBox):
                action = combo.currentData()
                self.cohesive_bundles[row]["action"] = action
                folder_rel = self.cohesive_bundles[row].get("folder_rel", "")
                self.bundle_action_changed.emit(folder_rel, action)
                # Re-render DEPOIS tree live to reflect user decision
    ...
    def _populate_tree_before(self, total: int):
        ...
        root_before.setText(0, f"📁 {self.root_dir.name} ({total})")
        root_before.setFont(0, QFont("Inter", 9, QFont.Weight.Bold))
        root_before.setData(0, Qt.ItemDataRole.UserRole, {"type": "folder", "folder_name": ".", "abs_path": str(self.root_dir), "is_source": True, "count": total})

        bundle_folder_names = {b.get("folder_rel", ""): b for b in self.cohesive_bundles}
        ...
        for folder_name, f_list in sorted(source_groups.items()):
            ...
            if folder_node != root_before:
                ...
                folder_node.setFont(0, QFont("Inter", 11, QFont.Weight.Medium))
            ...
            for it in f_list:
                ...
                status_icon = "📄" if it.get("status") == "identificado" else "❓"
                f_item.setText(0, f"{status_icon} {f_name}")
    ...
    def _populate_tree_after(self, total: int):
        ...
        root_after.setText(0, f"📁 Indexo_Files ({tr('summary.classified_count', count=total)})")
        root_after.setFont(0, QFont("Inter", 9, QFont.Weight.Bold))
        root_after.setData(0, Qt.ItemDataRole.UserRole, {"type": "folder", "folder_name": "Indexo_Files", "is_source": False, "count": total})

        bundle_action_map = {b.get("folder_rel", ""): b.get("action", "move_parent") for b in self.cohesive_bundles}
        bundle_obj_map = {b.get("folder_rel", ""): b for b in self.cohesive_bundles}

        # Structure: Category -> (Bundles, LooseTags)
        # dest_categories: { cat_name: { "bundles": { bundle_folder: [items] }, "tags": { tag_name: [items] } } }
        dest_categories: Dict[str, Dict[str, Dict[str, List[Dict[str, Any]]]]] = {}

        # Include user tags even if empty
        if self.user_tags:
        ...
        for it in self.items:
            ...
            # If NOT part of a cohesive bundle marked to move as parent:
            # Files with confidence < threshold or status != "identificado" MUST NOT be organized in preview!
            ...
            if is_in_bundle and bundle_action == "move_parent":
                ...
                # User chose to keep bundle folder in place; don't place in Indexo_Files tree
                continue
            else:
                # Loose file or disassembled bundle with confidence >= 0.65
        ...
        for cat_name, cat_data in sorted(dest_categories.items()):
            ...
            # 1. Cohesive Bundles inside this category (e.g. PEAK -> PEAK/PEAK.exe, assets)
            for b_folder, f_list in sorted(cat_data["bundles"].items()):
                ...
                b_node.setData(0, Qt.ItemDataRole.UserRole, {
                    "type": "folder",
                ...
                for it in f_list:
                    ...
                    conf_badge = f"[{conf_pct:.0f}%]" if conf_pct > 0 else f"[{tr('status.pending')}]"
                    rel_p = Path(it.get("rel_path", ""))
                    # Show subpath inside bundle
                    sub_name = rel_p.name if len(rel_p.parts) <= 2 else f".../{rel_p.name}"
                    file_icon = "🎮" if it.get("bundle_type") == "game" else "📄"
                    f_item.setText(0, f"{file_icon} {sub_name}  {conf_badge}")
                    f_item.setData(0, Qt.ItemDataRole.UserRole, {
                        "type": "file",
                        "abs_path": it.get("abs_path"),
                        "item": it
            ...
            # 2. Loose tags inside this category
            for tag_name, f_list in sorted(cat_data["tags"].items()):
                ...
                if tag_node != cat_node:
                    tag_node.setText(0, f"🏷️ {tag_name} ({tr('summary.files_count', count=len(f_list))})")
                    tag_node.setFont(0, QFont("Inter", 9, QFont.Weight.Medium))
                    tag_node.setData(0, Qt.ItemDataRole.UserRole, {
                        "type": "folder",
                ...
                for it in f_list:
                    ...
                    file_icon = "📄" if it.get("status") == "identificado" else "❓"
                    f_item.setText(0, f"{file_icon} {sug_name}  {conf_badge}")
                    f_item.setData(0, Qt.ItemDataRole.UserRole, {
                        "type": "file",
                        "abs_path": it.get("abs_path"),
                        "item": it
    ...
    def prompt_create_folder_in_source(self, target_parent: Optional[str] = None):
        ...
        if not base_dir or not base_dir.exists():
            QMessageBox.warning(self, "Indexo", "Selecione uma pasta de trabalho primeiro.")
            return

        name, ok = QInputDialog.getText(self, "Nova Pasta", "Nome da nova pasta:")
        if ok and name.strip():
            new_path = base_dir / name.strip()
            try:
                new_path.mkdir(parents=True, exist_ok=True)
                QMessageBox.information(self, "Indexo", f"Pasta '{name.strip()}' criada com sucesso!")
                self.refresh_requested.emit()
            except Exception as e:
                QMessageBox.critical(self, "Erro", f"Falha ao criar pasta: {e}")
