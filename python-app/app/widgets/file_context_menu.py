...
class FileTagEditDialog(QDialog):
    ...
    def init_ui(self):
        ...
        layout.addWidget(QLabel("<b>Categoria de Destino:</b>"))
        ...
        self.chk_learn = QPushButton("Auto-Aprendizado Ativo (Aprender com este arquivo)")
...
class FileMoveCategoryDialog(QDialog):
    ...
    def init_ui(self):
        self.setWindowTitle(f"📁 Mover Arquivo — {self.filename}")
        ...
        layout.addWidget(QLabel("<b>Escolha a Nova Categoria de Destino:</b>"))
        self.combo_cat = QComboBox()
        self.combo_cat.setEditable(True)
        categories = [
            tr("cat.faturas_boletos"),
...
class FilePropertiesDialog(QDialog):
    ...
    def init_ui(self):
        p = Path(self.file_path)
        self.setWindowTitle(f"Propriedades — {p.name}")
        ...
        if p.exists():
            ...
            add_row("Tamanho", size_str)
            add_row("Criado em", format_timestamp(st.st_ctime))
            add_row("Modificado em", format_timestamp(st.st_mtime))
            add_row("Extensão", p.suffix.upper() or "Sem extensão")
        ...
        if self.item_data:
            ...
            add_row("Categoria Indexo", cat)
            add_row("Tag Semântica", tag)
            add_row("Confiança", format_percentage(conf))
            add_row("Nome Sugerido", sug)
        ...
        # Bottom buttons
        btn_layout = QHBoxLayout()
        btn_copy = QPushButton("Copiar Caminho")
