...
class FolderReviewView(QWidget):
    ...
    def init_ui(self):
        ...
        self.btn_apply = QPushButton("Mover Apenas (Sem Renomear)")
    ...
    def load_from_items(self, items: List[Dict[str, Any]], root_dir: Path):
        ...
        for it in items:
            ...
            self.move_plan.append({
                ...
                "status": "Pronto para mover" if it.get("status") == "identificado" else "Pendente",
    ...
    def apply_moves(self):
        ...
        if not to_move:
            QMessageBox.information(self, "Indexo", "Nenhum arquivo selecionado para mover.")
            return

        msg = f"Deseja realmente mover os {len(to_move)} arquivos selecionados para suas respectivas pastas no disco, mantendo seus nomes originais intactos?"
        ...
        QMessageBox.information(self, "Indexo", f"{moved} arquivos foram movidos com sucesso para suas respectivas pastas, mantendo seus nomes originais intactos!")
        self.scan_and_plan()
