...
class TrashView(QWidget):
    ...
    def apply_filter(self):
        ...
        self.lbl_stats.setText(f"{tr('nav.trash')} ({len(self.marked_files)} files · {total_mb:.1f} MB)" if LanguageManager.get_instance().current_language == "enUS" else f"Lixeira Virtual ({len(self.marked_files)} arquivos marcados · {total_mb:.1f} MB)")
