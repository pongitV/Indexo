...
def main():
    ...
    # 4. Onboarding Wizard on First Execution (Section 5.A)
    ...
    if first_run:
        ...
        if wizard.exec():
            if wizard.selected_folder:
                default_folder = Path(wizard.selected_folder)
        settings_mgr.set("first_run", False)

    # 5. Launch Main Window
    window = MainWindow()
    if default_folder and default_folder.exists():
        window.set_active_folder(default_folder)
